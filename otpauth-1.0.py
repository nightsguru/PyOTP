...
if __name__ == "__main__":
    # Secret key (usually generated by the server and encoded in Base32)
    secret_key = "JBSWY3DPEHPK3PXP" # Base32
    
    # Create a TOTP instance
    totp = TOTP(secret=secret_key, issuer="MyService", label="alice@google.com")
    
    print("URI for Google Authenticator:", totp.to_uri())
    ...
    # Validate the code
    # window=1 allows accepting codes that were valid 1 period ago (or will be in the next)
    # this compensates for time synchronization issues between client and server
    is_valid = totp.validate(code, window=1)
    print("Is code valid:", is_valid is not None)

    # Parse URI
    parsed_totp = parse_uri("otpauth://totp/Example:alice@google.com?secret=JBSWY3DPEHPK3PXP&issuer=Example")
    print("\nParsed from URI code:", parsed_totp.generate())
